...
");")
cur.execute("""
CREATE TABLE IF NOT EXISTS ipfecha (
...
for elem in datos_usuarios["usuarios"]:
    clave = list(elem.keys())[0]
    cur.execute("INSERT OR IGNORE INTO usuarios(username, telefono, passwordHash, provincia, permisos)"
                "VALUES ('%s', '%d', '%s', '%s', '%d')" %
                (clave, elem[clave]['telefono'], elem[clave]['contrasena'], elem[clave]['provincia'],
                 elem[clave]['permisos']))
    cur.execute("INSERT OR IGNORE INTO emails(username, total, phishing, clickados)"
                "VALUES ('%s', '%d', '%d', '%d')" %
                (clave, elem[clave]["emails"]['total'], elem[clave]["emails"]['phishing'], elem[clave]["emails"]['clickados'],))
